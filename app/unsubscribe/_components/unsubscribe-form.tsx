...
import { CheckCircle2, AlertCircle, ArrowRight, Send, Loader } from "lucide-react";
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/subscribers/unsubscribe", {
        ...
        body: JSON.stringify({ email, feedback, staySubscribed }),
      });
    ...
    <div className="overflow-hidden rounded-xl bg-white shadow-lg">
      ...
      <div className="bg-[#11922f]/10 p-4">
        <div className="h-2.5 w-full rounded-full bg-gray-200">
          <div
            className="h-2.5 bg-gradient-to-r from-purple-200/50 to-purple-100 rounded-full transition-all duration-500 ease-in-out"
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-end gap-3">
                  <button
                    ...
                    disabled={isLoading}
                  >
                    {isLoading ? <Loader className="mr-2 h-4 w-4 animate-spin" /> : <CheckCircle2 className="mr-2 h-4 w-4" />}
                    Stay Subscribed
                  </button>

                  <button
                    type="submit"
                    className="flex items-center justify-center rounded-md bg-white/5 text-white hover:text-white border-purple-500/20 hover:bg-purple-500/10 transition-colors"
                    disabled={isLoading}
                  >
          ...
          {step === 2 && (
            ...
            >
              ...
              <h2 className="mb-2 text-2xl font-bold text-gray-800">
                {staySubscribed ? "Thank you for your feedback!" : "You've been unsubscribed"}
              </h2>
              ...
              <div className="flex flex-col justify-center gap-4 sm:flex-row">
                <Link
                  href="/"
                  className="inline-block rounded-md bg-white/5 text-white hover:text-white border-purple-500/20 hover:bg-purple-500/10"
                >
                  Return to Homepage
                </Link>
                ...
                {!staySubscribed && (
                  <button
                    ...
                    disabled={isLoading}
                  >
                    {isLoading ? <Loader className="mx-auto h-4 w-4 animate-spin" /> : "Resubscribe"}
                  </button>
                )}
              </div>
            </motion.div>
